...
A process binding may opt into `execution_target=omen`, or set `MCP_DEFAULT_EXECUTION_TARGET=omen` for that binding, when Windows has the canonical private OMEN execution owner available. OMEN execution is carried through `omen_exec.py` over the private-LAN SSH route; the MCP endpoint, OAuth identity, receipt/control directory, and public serving path remain Windows-owned. OMEN is not exposed as a public MCP endpoint. Remote OMEN execution currently accepts structured executable+argv only; script, stdin, and arbitrary environment injection are rejected, and OMEN starts default to the maximum bounded initial wait so ordinary commands normally complete without `read_output` polling.
