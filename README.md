...
Minimal authenticated Streamable HTTP MCP for the local Windows shell/process control boundary. Current serving ports, tasks, runtime roots, per-binding execution targets, and rollback lanes are machine-specific and must be read from the owning `mcp-current-topology.json` / recovery state plus live evidence; this package does not hardcode a production listener as current truth. Historical VPS/WireGuard/tunnel tooling remains in the repository for recovery/research history but is not part of the current Windows-owned serving boundary or the one-package installer.
...
A process binding may opt into `execution_target=omen`, or set `MCP_DEFAULT_EXECUTION_TARGET=omen` for that binding. The production architecture is MCP-to-MCP: the Windows public/OAuth edge delegates process tools to an MCP server running on OMEN through a private loopback tunnel configured with `MCP_OMEN_MCP_URL`; OMEN launches the structured argv locally and reports `execution_transport=native-mcp`. SSH is not part of the serving execution path. `omen_exec.py` remains supported only as an explicit rollback/bootstrap compatibility adapter and reports `execution_transport=ssh-adapter`. Remote OMEN execution accepts structured executable+argv only; script, stdin, and arbitrary environment injection are rejected.
...
## Native OMEN MCP execution

Run the MCP server on OMEN with `MCP_NATIVE_OMEN_HOST=1`, `MCP_DEFAULT_EXECUTION_TARGET=omen`, and loopback-only internal MCP trust. A Windows public/OAuth edge delegates to it with `MCP_OMEN_MCP_URL=http://127.0.0.1:<private-tunnel-port>/mcp`. Structured `start_process`, `read_output`, and `kill_process` stay on the OMEN MCP owner and report `execution_transport=native-mcp` plus an `execution_serving_identity`; SSH is not part of that serving path. When `OmenMcpUrl` is supplied to `prepare-frozen-home-direct.ps1`, the frozen Windows launcher leaves `MCP_OMEN_EXEC_PATH` empty.
