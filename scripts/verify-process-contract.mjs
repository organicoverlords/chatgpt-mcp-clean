...
const actualTools = Object.entries(server._registeredTools).sort(([a],[b])=>a.localeCompare(b)).map(([name,tool])=>({name,description:tool.description||"",inputSchema:z.toJSONSchema(tool.inputSchema)}));
...
const actualHash=createHash("sha256").update(readFileSync(resolve("dist/server.js"))).digest("hex");
...
console.log(`PASS process_contract_guard tools=${actualTools.map(t=>t.name).join(",")} server_sha256=${actualHash}`);
