...
await mod.serveLibrarySpoolBridgeAck({ query: { token, id: nextRes.body.id } }, staleAckRes);
assert.equal(staleAckRes.statusCode, 400, "stale widgets without uploaded file id must not consume proofs");
assert.equal((await readdir(queue)).length, 1, "stale widget acknowledgement must leave proof queued");
const ackRes = new FakeResponse();
await mod.serveLibrarySpoolBridgeAck({ query: { token, id: nextRes.body.id, file_id: "file_bridge_acceptance" } }, ackRes);
...
console.log("PASS persistent_visual_bridge mount_action=mount_visual_proof_bridge start_widget=false mount_consumes_proof=false polling=true exact_hash=true resource_message=false library_upload=true per_proof_upload=true ack_after_message=true");
