...
while (pages.at(-1).next_action === "READ_SAME_PROCESS_ID") {
  pages.push(manager.read(first.process_id, 60_000));
  assert.ok(pages.length < 10, "paging terminates");
}

for (const page of pages) {
  if (!page.output_page) continue;
  assert.ok(page.output_page.page_chars <= 60_000, "logical page never exceeds 60k");
  assert.equal(page.output_page.page_limit, 60_000, "page uses current 60k read contract");
}
...
console.log(`PASS lossless_output_60k pages=${pages.length} chars=${expected.length} first_page=${pages[0].stdout.length}`);
