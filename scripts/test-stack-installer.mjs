...
assert.match(start, /MCP_LOCAL_FILE_TOOL_NAME = 'upload_local_file'/);
assert.match(start, /MCP_LIBRARY_SPOOL_BRIDGE = '0'/);
assert.match(start, /protect-oauth-state\.ps1/);
assert.match(installSource, /basic_auth/);
assert.match(installSource, /hash-password --plaintext/);
assert.match(installSource, /X-MCP-Owner-Authorized 1/);
assert.match(installSource, /header_up -Authorization/);
assert.match(installSource, /header_up -X-MCP-Owner-Authorized/);
assert.match(installSource, /owner-auth\.json/);
assert.match(installSource, /protect-oauth-state\.ps1/);
assert.match(installSource, /MCP_LOCAL_FILE_TOOL_NAME = 'upload_local_file'/);
assert.match(installSource, /MCP_LIBRARY_SPOOL_BRIDGE = '0'/);
assert.doesNotMatch(installSource, /remote_ip private_ranges/);
assert.deepEqual(processContract.map((tool) => tool.name).sort(), ['download_chatgpt_file','kill_process','read_output','start_process','upload_local_file']);
assert.match(serverSource, /registerFileTransferTools\(server, callerId\)/);
assert.match(fileTransferSource, /FILE_TRANSFER_WIDGET_URI/);
...
try {
  ...
  assert.equal(plan.tool_count, 5);
  assert.equal(plan.library_delivery, 'explicit file tools');
