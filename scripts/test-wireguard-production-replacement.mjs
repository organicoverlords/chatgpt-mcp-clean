...
assert.ok(launcher.includes("$memory.MemoryPriority = 5"));
assert.ok(launcher.includes("SetProcessInformation($Process.Handle, 0,"));
assert.ok(launcher.includes("$ioPriority = [uint32]2"));
assert.ok(launcher.includes("NtSetInformationProcess($Process.Handle, 33,"));
