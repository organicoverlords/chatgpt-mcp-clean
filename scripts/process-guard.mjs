...
try {
  ...
  const cleanupManager = new ProcessManager({ receiptDirectory: hostAdmissionDirectory, maxLiveTotal: 2 });
  for (const process of hostAdmissionProcesses) await cleanupManager.kill(process.process_id).catch(() => undefined);
...
console.log("PASS process guard enforces duplicate reuse, per-caller and shared-host live concurrency, restart receipts, cross-clone control, fast-start collapse, compact no-change waits, nonblocking zero-wait, and bounded-wait behavior, and time-based receipt retention");
