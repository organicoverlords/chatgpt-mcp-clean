...
const startReadRevisionManager = new ProcessManager();
let startReadRevisionProcess;
try {
  startReadRevisionProcess = await startReadRevisionManager.startWithWait(
    "Write-Output 'START_PACKET_VISIBLE'; Start-Sleep -Seconds 3",
    undefined,
    "caller_start_read_revision_test",
    150,
  );
  assert.equal(startReadRevisionProcess.running, true, JSON.stringify(startReadRevisionProcess));
  assert.match(startReadRevisionProcess.stdout, /START_PACKET_VISIBLE/);
  const followupStartedAt = Date.now();
  const followup = await startReadRevisionManager.readWithWait(startReadRevisionProcess.process_id, 6_000, 250);
  ...
  assert.ok(followupDurationMs >= 180, `first positive read after start_process replayed already-delivered output (${followupDurationMs}ms)`);
  assert.ok(followupDurationMs < 1_000, `first positive read after start_process exceeded its wait bound (${followupDurationMs}ms)`);
  const snapshotReplay = await startReadRevisionManager.readWithWait(startReadRevisionProcess.process_id, 6_000, 0);
  assert.match(snapshotReplay.stdout, /START_PACKET_VISIBLE/, "nonblocking snapshots must still expose retained start output");
} finally {
  if (startReadRevisionProcess) await startReadRevisionManager.kill(startReadRevisionProcess.process_id).catch(() => undefined);
}
