...
try{
 ...
 const expected=["actor_status","busy_claim","busy_list","busy_release","edit_file","git_add","git_checkout","git_commit","git_diff","git_log","git_pull","git_push","git_status","glob","grep","incident_report_skill","issue_close","issue_comment","issue_list","list_directory","orchestration_skill","orchestration_status","process_output","process_status","read_text_file","run_command","start_process","stop_process","write_file"].sort();
 ...
 const orch=await rpc({jsonrpc:"2.0",id:21,method:"tools/call",params:{name:"orchestration_skill",arguments:{}}},sid);const orchText=orch.json.result.content[0].text;if(!orchText.includes("Do not read every project AGENTS.md just to orient"))throw new Error("orchestration skill missing orient rule");if(orchText.includes(root))throw new Error("orchestration skill exposed server path");
 const incident=await rpc({jsonrpc:"2.0",id:22,method:"tools/call",params:{name:"incident_report_skill",arguments:{}}},sid);const incidentText=incident.json.result.content[0].text;if(!incidentText.includes("incident-contract.md")||!incidentText.includes("verify_incident_capture.py"))throw new Error("incident skill incomplete");if(incidentText.includes(root))throw new Error("incident skill exposed server path");
 const compat=await rpc({jsonrpc:"2.0",id:23,method:"tools/call",params:{name:"orchestration_status",arguments:{}}},sid);if(!compat.json.result.content[0].text.includes("Do not read every project AGENTS.md just to orient"))throw new Error("orchestration compatibility alias broken");
 ...
 console.log(`PASS tools=${names.length} skills=2 actor=${JSON.parse(actorText).actor_id} recovery=ok oauth_refresh=ok no_instructions=ok no_path_leak=ok secret_filter=ok`);
