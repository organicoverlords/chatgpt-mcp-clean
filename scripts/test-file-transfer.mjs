...
try {
  ...
  const activeWidgetResource = listedResources.resources.find((resource) => resource.uri === FILE_TRANSFER_WIDGET_URI);
  assert.equal(activeWidgetResource?.mimeType, "text/html;profile=mcp-app", "official registerAppResource must advertise MCP Apps MIME type in resources/list");
...
assert.match(widget, /uploadFileAvailable/);
assert.match(widget, /selectFilesAvailable/);
assert.match(widget, /getFileDownloadUrlAvailable/);
